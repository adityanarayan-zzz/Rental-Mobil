...
const dummyMobil: Mobil[] = [
  { id: 1, nama: "Toyota Innova", tipe: "MPV", harga: 450000, kursi: 7, totalUnit: 5, unitTersedia: 5 },
  { id: 2, nama: "Toyota Avanza", tipe: "MPV", harga: 300000, kursi: 7, totalUnit: 8, unitTersedia: 8 },
  { id: 3, nama: "Toyota Fortuner", tipe: "SUV", harga: 650000, kursi: 7, totalUnit: 3, unitTersedia: 3 },
  { id: 4, nama: "Honda Jazz", tipe: "Hatchback", harga: 250000, kursi: 5, totalUnit: 4, unitTersedia: 4 },
  { id: 5, nama: "Toyota Hiace", tipe: "Van", harga: 750000, kursi: 14, totalUnit: 2, unitTersedia: 2 },
];

type ModalType = "add" | "edit" | "delete" | null;

const emptyForm = { nama: "", tipe: "MPV", harga: 0, kursi: 5, totalUnit: 1, unitTersedia: 1, gambar: "" };

export default function AdminDashboard() {
  const navigate = useNavigate();
  const [mobils, setMobils] = useState<Mobil[]>(dummyMobil);
  ...
  function openEdit(m: Mobil) {
    setSelected(m);
    setForm({ nama: m.nama, tipe: m.tipe, harga: m.harga, kursi: m.kursi, totalUnit: m.totalUnit, unitTersedia: m.unitTersedia, gambar: m.gambar || "" });
    setModal("edit");
  }
  ...
  function handleAdd() {
    const newMobil: Mobil = { ...form, id: Date.now() };
    setMobils([...mobils, newMobil]);
    setModal(null);
  }

  function handleEdit() {
    setMobils(mobils.map((m) => m.id === selected?.id ? { ...m, ...form } : m));
    setModal(null);
  }

  function handleDelete() {
    setMobils(mobils.filter((m) => m.id !== selected?.id));
    setModal(null);
  }

  function handleUnitChange(id: number, delta: number) {
    setMobils(mobils.map((m) => m.id === id ? { ...m, totalUnit: Math.max(0, m.totalUnit + delta), unitTersedia: Math.max(0, Math.min(m.unitTersedia, m.totalUnit + delta)) } : m));
  }
  ...
  const totalMobil = mobils.length;
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .admin-sidebar {
          width: 240px;
          background: linear-gradient(180deg, #0f1f5c 0%, #1a3fa8 100%);
          min-height: 100vh;
          display: flex;
        ...
        <main className="admin-main">
          ...
          <div className="admin-stats">
            <div className="admin-stat-card">
              ...
              <div className="admin-stat-value">{totalMobil}</div>
            ...
            <div className="admin-stat-card">
              ...
              <div className="admin-stat-value" style={{ color: "#1d86d7" }}>{sedangDisewa}</div>
                  ...
                  <tbody>
                    {filtered.map((m, i) => (
                      <tr key={m.id}>
                        ...
                        <td style={{ fontWeight: 700, color: "#1a3fa8" }}>
                          Rp {m.harga.toLocaleString("id-ID")}
                        </td>
                        <td>{m.kursi} kursi</td>
                        <td>
                          <div className="unit-control">
                            <button className="unit-btn" onClick={() => handleUnitChange(m.id, -1)}>−</button>
            ...
            <div className="modal-form">
              <div className="modal-grid">
                ...
                <div className="modal-field">
                  ...
                  <input className="modal-input" type="number" min={0} value={form.totalUnit} onChange={(e) => setForm({ ...form, totalUnit: Number(e.target.value), unitTersedia: Number(e.target.value) })} />
                </div>
              </div>
              <div className="modal-actions">
                <button className="btn-cancel" onClick={() => setModal(null)}>Batal</button>
                <button className="btn-confirm" onClick={modal === "add" ? handleAdd : handleEdit}>
      ...
      {modal === "delete" && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setModal(null)}>
          <div className="modal-card">
            ...
            <div className="modal-actions">
              <button className="btn-cancel" onClick={() => setModal(null)}>Batal</button>
              <button className="btn-confirm-delete" onClick={handleDelete}>Hapus</button>
            </div>
          </div>
        </div>
      )}
